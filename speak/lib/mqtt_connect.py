...
class Mqtt:
    ...
    def on_disconnect(self,client, userdata, rc):
        print "To disconnect. Reason : " + str(rc)
    ...
    def connect(self, option):
        ...
        host_statue = 1
        while host_statue :
            for host in self.mqtt_options['host']:
                try:
                    print "Connecting to "+ host +" ....."
                    self.mqtt_client.connect(host, self.mqtt_options["port"], 60)
                    host_statue = 0
                    break
                except Exception as e:
                    print "Error occurred while trying to connect to QIoT Suite broker. Reason : " + str(e.args)
                    print "Retry next Host....."
                    #sys.exit(e)
        print "finish setup"
    ...
    def publish_by_id(self, resource_id, value):
        ...
        for res in resources:
            if (resource_id == str(res["resourceid"])):
                vals = "{\"value\":" +"\""+ value + "\"" +"}"
                print "NOW TOPIC_NAME :" + str(res["topic"]) + " MESSAGE : " + vals
